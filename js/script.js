...
async function watchAdForQuest(questId, adType, adLimit, button, li, userData) {
    ...
    try {
        await showAd(adType); // Assuming showAd is globally defined and returns promise
...
// Keep your existing showAd function here, it seems okay functionally
// Just ensure it's defined globally or accessible where needed.
async function showAd(adType) { // Changed questId param to adType for clarity
    ...
    return new Promise((resolve, reject) => {
        ...
        // Check if SDK function exists (using the specific function name from HTML)
        ...
        try {
            ...
            // --- Trigger Ad Based on Type (excluding 'inApp') ---
             // Assuming adType parameter passed to showAd matches Monetag types like 'pop' or undefined for interstitial
             if (adType === 'pop') { // Example check for popup type
                 adPromise = window.show_9180370('pop');
                 adTriggered = true;
                 requiresPromiseHandling = true;
             } else { // Default to standard interstitial if adType is not 'pop' or is undefined/unrecognized
                  adPromise = window.show_9180370(); // Default interstitial call
                  adTriggered = true;
                  requiresPromiseHandling = true;
             }
            ...
            if (requiresPromiseHandling && adPromise && typeof adPromise.then === 'function') {
                ...
                adPromise.then(() => {
                 ...
                 console.warn(`[AD WARN] SDK call for ${adType} was triggered but did not return a standard promise. Relying on timeout.`);
                  // If no promise, we might need to resolve based on an event or assume success after timeout (less ideal)
                  // For Monetag, often the promise IS the primary mechanism. If not, Monetag might use callbacks.
                  // Check Monetag docs if promises aren't returned reliably.
            }
...
function updateQuestItemUI(questId, listItemElement) {
    ...
    const currentTime = new Date();
    const lastClaimedTime = adProgress.lastClaimed ? new Date(adProgress.lastClaimed) : null; // Use adProgress for cooldown
    const cooldownPeriod = 3600 * 1000; // 1 hour
    const timeSinceLastClaim = lastClaimedTime ? currentTime - lastClaimedTime : Infinity;
    const isCooldownOver = timeSinceLastClaim >= cooldownPeriod;
    ...
    if (isEffectivelyClaimed && (!isAdBased || !isCooldownOver)) { // Claimed (non-ad) OR Claimed Ad still in cooldown
        ...
        button.disabled = true;
    } else if (isCompleted && !isEffectivelyClaimed) { // Ready to claim (Ad completed or non-ad not claimed yet)
        button.className = 'claim-button active';
        button.textContent = 'Claim';
        button.disabled = false;
    } else { // Not completed (Ad) or Cooldown is over for Ad (can watch again)
        button.className = 'go-button';
        button.textContent = isAdBased ? (listItemElement.querySelector('span')?.textContent || 'Watch Ad') : (listItemElement.querySelector('span')?.textContent || 'GO'); // Try to get original action text
        button.disabled = false;
    }
...
async function updateInviteSectionUI() {
    ...
    if (claimHistory.length === 0) {
        ...
        claimHistory.sort((a, b) => (b.claimTime?.toDate?.() || 0) - (a.claimTime?.toDate?.() || 0)) // Handle Firestore Timestamp objects
            .forEach(record => {
    ...
    const inviteRecords = userData.inviteRecords || [];
     if (inviteRecords.length === 0) {
        ...
        inviteRecordPlaceholderEl.style.display = 'none';
         ...
         inviteRecords.sort((a, b) => (b.joinTime?.toDate?.() || 0) - (a.joinTime?.toDate?.() || 0)) // Handle Firestore Timestamp objects
             .forEach(record => {
                const div = document.createElement('div');
                div.className = 'record-item';
...
async function handleClaimCreditsClick(event) {
    ...
    try {
        ...
        await db.runTransaction(async (transaction) => {
            ...
            claimButton.textContent = 'Claiming...'; // Update text inside transaction if possible
    ...
    // No finally needed for button state, updateInviteSectionUI handles it based on refreshed data
}
...
async function initializeApp() {
    ...
    try {
        if (typeof window.show_9180370 === 'function') {
            const autoInAppSettings = { /* ... your settings ... */ };
            debugLog('[AD INIT] Initializing automatic In-App ads...');
